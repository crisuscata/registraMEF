...
var activarmsProyectoInversionUrl = contexto+"/rs/ctrlmsProyectoInversion/activarmsProyectoInversion";
var eliminarListamsProyectoInversionUrl = contexto+"/rs/ctrlmsProyectoInversion/eliminarListamsProyectoInversion";
...
//		    				$scope.datos.push(dato); 
	   ...
	   $scope.eliminarmsProyectoInversion = function(ev,msProyectoInversionBk){		
            ...
            $scope.activarmsProyectoInversion = function(ev,msProyectoInversionBk){		
				    ev.target.disabled = true;
				    $scope.setMsProyectoInversionModelo(msProyectoInversionBk);
				    var datainsert = angular.toJson($scope.msProyectoInversionModelo);
		 			console.log("Activar = "+datainsert);	
	        		$http.post(activarmsProyectoInversionUrl,datainsert,{headers: {'Content-Type': 'application/json'}}).then(function(res){
						var dato = res.data;
						var instrumentos = $scope.datos;
				        var index = $scope.datos.findIndex(obj => obj.idProyecto === dato.idProyecto);
						console.log("INDEX " + index);
			...
			if(typeof(dato) != 'undefined'){
					        ...
					        .parent(angular.element(document.body))
					        .clickOutsideToClose(true)
					        .title('Activar Proyectos De Inversión')
					        .textContent(dato)
					        .ariaLabel('ERROR')
					        .ok('OK')
					        .targetEvent(ev)
            ...
            $scope.eliminarListamsProyectoInversion = function(ev){		
				    ev.target.disabled = true;
					...
					return;
				             ...
				             listaids.push($scope.selected[i].idProyecto);
			                 }
                                    } 
				    
				    var datainsert = angular.toJson(listaids);
		 			console.log("Elimiar = "+datainsert);	
	        		$http.post(eliminarListamsProyectoInversionUrl,datainsert,{headers: {'Content-Type': 'application/json'}}).then(function(res){
						var dato = res.data;
						$mdDialog.show(
					$mdDialog.alert()
					.parent(angular.element(document.body))
		...
		$scope.showConfirm = function(ev, msProyectoInversionBk) {
                ...
                $scope.showConfirmActivar = function(ev, msProyectoInversionBk) {
				    var confirm = $mdDialog.confirm()
				      .title('Activar Proyectos De Inversión')
				      .textContent('Esta usted seguro de activar el registro?')
				      .ariaLabel('Lucky day')
				    ...
				    $mdDialog.show(confirm).then(function () {
				      $scope.status = 'SI';
				      $scope.activarmsProyectoInversion(ev, msProyectoInversionBk);
				    }, function () {
				      $scope.status = 'NO';
				    });
				  };
                ...
                $scope.showConfirmLista = function(ev) {
				    ...
				    $mdDialog.show(confirm).then(function () {
				      $scope.status = 'SI';
				      $scope.eliminarListamsProyectoInversion(ev);
				    }, function () {
				      $scope.status = 'NO';
				    });
				  };		  
