...
var activarmsPaisesUrl = contexto+"/rs/ctrlmsPaises/activarmsPaises";
var eliminarListamsPaisesUrl = contexto+"/rs/ctrlmsPaises/eliminarListamsPaises";
...
estado: null,
	  ...
	  $scope.clearmsPaises = function(){
		    ...
		    $scope.msPaisesModelo.fechaCrea = new Date();$scope.msPaisesModelo.fechaModif = new Date();$scope.msPaisesModelo.estado = null;
	  ...
	  $scope.setMsPaisesModelo = function(msPaisesBk) {
		  $scope.msPaisesModelo.idpais = msPaisesBk.idpais;
			...
			$scope.msPaisesModelo.fechaCrea = msPaisesBk.fechaCrea;
                        if(!$scope.isNull($scope.msPaisesModelo.fechaCrea) && !isNaN($scope.msPaisesModelo.fechaCrea)){
			    $scope.msPaisesModelo.fechaCrea = new Date($scope.msPaisesModelo.fechaCrea);
		        }
$scope.msPaisesModelo.fechaModif = msPaisesBk.fechaModif;
                        if(!$scope.isNull($scope.msPaisesModelo.fechaModif) && !isNaN($scope.msPaisesModelo.fechaModif)){
			    $scope.msPaisesModelo.fechaModif = new Date($scope.msPaisesModelo.fechaModif);
		        }
...
//		    				$scope.datos.push(dato); 
	   ...
	   $scope.eliminarmsPaises = function(ev,msPaisesBk){		
            ...
            $scope.activarmsPaises = function(ev,msPaisesBk){		
				    ev.target.disabled = true;
				    $scope.setMsPaisesModelo(msPaisesBk);
				    var datainsert = angular.toJson($scope.msPaisesModelo);
		 			console.log("Activar = "+datainsert);	
	        		$http.post(activarmsPaisesUrl,datainsert,{headers: {'Content-Type': 'application/json'}}).then(function(res){
						var dato = res.data;
						var instrumentos = $scope.datos;
				        var index = $scope.datos.findIndex(obj => obj.idpais === dato.idpais);
						console.log("INDEX " + index);
            ...
            $scope.eliminarListamsPaises = function(ev){		
				    ev.target.disabled = true;
					...
					return;
				             ...
				             listaids.push($scope.selected[i].idpais);
			                 }
                                    } 
				    
				    var datainsert = angular.toJson(listaids);
		 			console.log("Elimiar = "+datainsert);	
	        		$http.post(eliminarListamsPaisesUrl,datainsert,{headers: {'Content-Type': 'application/json'}}).then(function(res){
						var dato = res.data;
						$mdDialog.show(
					$mdDialog.alert()
					.parent(angular.element(document.body))
		...
		$scope.showConfirm = function(ev, msPaisesBk) {
                ...
                $scope.showConfirmActivar = function(ev, msPaisesBk) {
				    var confirm = $mdDialog.confirm()
				      .title('Activar Países')
				      .textContent('Esta usted seguro de activar el registro?')
				      .ariaLabel('Lucky day')
				    ...
				    $mdDialog.show(confirm).then(function () {
				      $scope.status = 'SI';
				      $scope.activarmsPaises(ev, msPaisesBk);
				    }, function () {
				      $scope.status = 'NO';
				    });
				  };
                ...
                $scope.showConfirmLista = function(ev) {
				    ...
				    $mdDialog.show(confirm).then(function () {
				      $scope.status = 'SI';
				      $scope.eliminarListamsPaises(ev);
				    }, function () {
				      $scope.status = 'NO';
				    });
				  };		  
