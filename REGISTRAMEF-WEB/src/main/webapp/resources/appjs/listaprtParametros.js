...
var activarprtParametrosUrl = contexto+"/rs/ctrlprtParametros/activarprtParametros";
var eliminarListaprtParametrosUrl = contexto+"/rs/ctrlprtParametros/eliminarListaprtParametros";
var editarprtParametrosUrl = contexto+"/rs/ctrlprtParametros/editarprtParametros/";
var listaPrtParametrosidparametroIdpadreUrl = contexto+"/rs/ctrlprtParametros/listaPrtParametrosIdparametroIdpadre";
var descargarUrl = contexto+"/rs/ctrlprtParametros/descargar/";

///URLs CARGA DE ARCHIVOS
var insertDocUrl = contexto+"/rs/ctrlprtParametros/insertarchivo";
///FIN URLs CARGA DE ARCHIVOS

var descargarvistaUrl = contexto+"/rs/ctrlprtParametros/descargarvista";
...
myapp.controller('ctrlListaprtParametros', ['$mdEditDialog', '$scope', '$timeout', '$http', '$mdDialog','$location','$routeParams', '$mdPanel',  function ($mdEditDialog, $scope, $timeout, $http, $mdDialog, $location, $routeParams, $mdPanel) {
	'use strict';
	 ...
	 $scope.options = {
//			    rowSelection: true,
			 rowSelection: false, 	  /* PURIBE 20012024 - INICIO */
			    multiSelect: true,
			    autoSelect: true,
			    decapitate: false,
	...
	$scope.loadprtParametross = function () {
	    //$scope.promise = $timeout(function () {
	    	...
	    	$scope.promise = $http.get(surl).then(function(res){
	    		 ...
	    		 console.log("Tiempo respuesta BD prtParametros " +tiempoenBD+" Tiempo en Paginar "+tiempoenproceso);
				},
	  ...
	  $scope.getURLParametros=function(){
		  ...
		  return listaprtParametrosUrl+$scope.getURLParametros();
	  }
	  ...
	  $scope.editprtParametros = function () {
		    ...
		    if(idparametro){
		    	$scope.cargarprtParametros(idparametro);
		    }
		   ///CARGAR COMPLEMENTOS 
...
estadoTxt: null,
	  ...
	  $scope.clearprtParametros = function(){
		    ...
		    $scope.prtParametrosModelo.fechaCrea = new Date();$scope.prtParametrosModelo.fechaModif = new Date();$scope.prtParametrosModelo.estado = null;
		    
                    // ADICIONALES
	            $scope.prtParametrosModelo.idpadreTxt = null;
$scope.prtParametrosModelo.estadoTxt = null;
	  ...
	  $scope.setPrtParametrosModelo = function(prtParametrosBk) {
		  $scope.prtParametrosModelo.idparametro = prtParametrosBk.idparametro;
			...
			$scope.prtParametrosModelo.idpadre = prtParametrosBk.idpadre;
			$scope.prtParametrosModelo.fechaCrea = prtParametrosBk.fechaCrea;
                        if(!$scope.isNull($scope.prtParametrosModelo.fechaCrea) && !isNaN($scope.prtParametrosModelo.fechaCrea)){
			    $scope.prtParametrosModelo.fechaCrea = new Date($scope.prtParametrosModelo.fechaCrea);
		        }
$scope.prtParametrosModelo.fechaModif = prtParametrosBk.fechaModif;
                        if(!$scope.isNull($scope.prtParametrosModelo.fechaModif) && !isNaN($scope.prtParametrosModelo.fechaModif)){
			    $scope.prtParametrosModelo.fechaModif = new Date($scope.prtParametrosModelo.fechaModif);
		        }
$scope.prtParametrosModelo.estado = prtParametrosBk.estado;
/* PURIBE 20012024 - INICIO*/
if (prtParametrosBk.estado==0){
	$scope.prtParametrosModelo.estado = false;
	}
	else{
		$scope.prtParametrosModelo.estado = true;
	}
/* PURIBE 20012024 - FIN*/
			
                        // ADICIONALES
	                $scope.prtParametrosModelo.idpadreTxt = prtParametrosBk.idpadreTxt;
$scope.prtParametrosModelo.estadoTxt = prtParametrosBk.estadoTxt;

			$scope.prtParametrosModelo.editopcion = prtParametrosBk.prtParametrosACL.editopcion;
		}
	  // ////////////////////////////////////////////////
	  $scope.editarPrtParametros = function(ev, prtParametrosBk) {		  
		    $scope.setPrtParametrosModelo(prtParametrosBk);		  
			$location.url('/editar/' + $scope.prtParametrosModelo.idparametro);
			$scope.nuevo = false;
	  }
	  ...
	  $scope.updateModel = function() {
		...
		$scope.estado = $scope.prtParametrosModelo.estado ? 3 : 0;
	};
		/* PURIBE 20012024 - FIN*/
	  ...
	  $scope.salvarPrtParametros = function(ev){		
					...
					/* PURIBE 20012024 - INICIO*/
					$scope.estado = $scope.prtParametrosModelo.estado ? 3 : 0;
					$scope.prtParametrosModelo.estado = $scope.estado
					/* PURIBE 20012024 - FIN*/
					/* PURIBE 25012024 - FIN*/
...
//		    				$scope.datos.push(dato); 
		    				...
		    				$scope.setPrtParametrosModelo(dato);
		    				...
		    				$mdDialog.show(
							        ...
							        .textContent("del Parámetros se guardó correctamente.")
							        .ariaLabel('ERROR')
							        .ok('OK')
							        .targetEvent(ev)
							    );
	   ...
	   $scope.eliminarprtParametros = function(ev,prtParametrosBk){		
            ...
            $scope.activarprtParametros = function(ev,prtParametrosBk){		
				    ev.target.disabled = true;
				    $scope.setPrtParametrosModelo(prtParametrosBk);
				    var datainsert = angular.toJson($scope.prtParametrosModelo);
		 			console.log("Activar = "+datainsert);	
	        		$http.post(activarprtParametrosUrl,datainsert,{headers: {'Content-Type': 'application/json'}}).then(function(res){
						var dato = res.data;
						var instrumentos = $scope.datos;
				        var index = $scope.datos.findIndex(obj => obj.idparametro === dato.idparametro);
						console.log("INDEX " + index);
            ...
            $scope.eliminarListaprtParametros = function(ev){		
				    ev.target.disabled = true;
					...
					return;
				             ...
				             listaids.push($scope.selected[i].idparametro);
			                 }
                                    } 
				    
				    var datainsert = angular.toJson(listaids);
		 			console.log("Elimiar = "+datainsert);	
	        		$http.post(eliminarListaprtParametrosUrl,datainsert,{headers: {'Content-Type': 'application/json'}}).then(function(res){
						var dato = res.data;
						$mdDialog.show(
					$mdDialog.alert()
					.parent(angular.element(document.body))
		...
		$scope.showConfirm = function(ev, prtParametrosBk) {
                ...
                $scope.showConfirmActivar = function(ev, prtParametrosBk) {
				    var confirm = $mdDialog.confirm()
				      .title('Activar Parámetros')
				      .textContent('Esta usted seguro de activar el registro?')
				      .ariaLabel('Lucky day')
				    ...
				    $mdDialog.show(confirm).then(function () {
				      $scope.status = 'SI';
				      $scope.activarprtParametros(ev, prtParametrosBk);
				    }, function () {
				      $scope.status = 'NO';
				    });
				  };
                ...
                $scope.showConfirmLista = function(ev) {
				    ...
				    $mdDialog.show(confirm).then(function () {
				      $scope.status = 'SI';
				      $scope.eliminarListaprtParametros(ev);
				    }, function () {
				      $scope.status = 'NO';
				    });
				  };		  
...
//SELECT INI
				...
				$scope.loadListaPrtParametrosIdpadre=function(){
					$http.get(listaPrtParametrosidparametroIdpadreUrl).then(function(res){
						$scope.listaPrtParametrosIdpadre = res.data; 
...
//	if($scope.isArray($scope.listaPrtParametrosIdpadre)){
//		var obj = $scope.listaPrtParametrosIdpadre.find(o => o.id === newValue);
