...
var activarmsMetaUrl = contexto+"/rs/ctrlmsMeta/activarmsMeta";
var eliminarListamsMetaUrl = contexto+"/rs/ctrlmsMeta/eliminarListamsMeta";
...
estadoTxt: null,
	  ...
	  $scope.clearmsMeta = function(){
		    ...
		    $scope.msMetaModelo.fechaCrea = new Date();$scope.msMetaModelo.fechaModif = new Date();$scope.msMetaModelo.estado = null;
		    
                    // ADICIONALES
	            $scope.msMetaModelo.idTipoServicioTxt = null;
$scope.msMetaModelo.idSistAdmiTxt = null;
$scope.msMetaModelo.idSedeTxt = null;
$scope.msMetaModelo.estadoTxt = null;
	  ...
	  $scope.setMsMetaModelo = function(msMetaBk) {
		  $scope.msMetaModelo.idMeta = msMetaBk.idMeta;
			...
			$scope.msMetaModelo.fechaCrea = msMetaBk.fechaCrea;
                        if(!$scope.isNull($scope.msMetaModelo.fechaCrea) && !isNaN($scope.msMetaModelo.fechaCrea)){
			    $scope.msMetaModelo.fechaCrea = new Date($scope.msMetaModelo.fechaCrea);
		        }
$scope.msMetaModelo.fechaModif = msMetaBk.fechaModif;
                        if(!$scope.isNull($scope.msMetaModelo.fechaModif) && !isNaN($scope.msMetaModelo.fechaModif)){
			    $scope.msMetaModelo.fechaModif = new Date($scope.msMetaModelo.fechaModif);
		        }
$scope.msMetaModelo.estado = msMetaBk.estado;
...
$scope.msMetaModelo.estadoTxt = msMetaBk.estadoTxt;
...
//		    				$scope.datos.push(dato); 
	   ...
	   $scope.eliminarmsMeta = function(ev,msMetaBk){		
            ...
            $scope.activarmsMeta = function(ev,msMetaBk){		
				    ev.target.disabled = true;
				    $scope.setMsMetaModelo(msMetaBk);
				    var datainsert = angular.toJson($scope.msMetaModelo);
		 			console.log("Activar = "+datainsert);	
	        		$http.post(activarmsMetaUrl,datainsert,{headers: {'Content-Type': 'application/json'}}).then(function(res){
						var dato = res.data;
						var instrumentos = $scope.datos;
				        var index = $scope.datos.findIndex(obj => obj.idMeta === dato.idMeta);
						console.log("INDEX " + index);
            ...
            $scope.eliminarListamsMeta = function(ev){		
				    ev.target.disabled = true;
					...
					return;
				             ...
				             listaids.push($scope.selected[i].idMeta);
			                 }
                                    } 
				    
				    var datainsert = angular.toJson(listaids);
		 			console.log("Elimiar = "+datainsert);	
	        		$http.post(eliminarListamsMetaUrl,datainsert,{headers: {'Content-Type': 'application/json'}}).then(function(res){
						var dato = res.data;
						$mdDialog.show(
					$mdDialog.alert()
					.parent(angular.element(document.body))
		...
		$scope.showConfirm = function(ev, msMetaBk) {
                ...
                $scope.showConfirmActivar = function(ev, msMetaBk) {
				    var confirm = $mdDialog.confirm()
				      .title('Activar Metas')
				      .textContent('Esta usted seguro de activar el registro?')
				      .ariaLabel('Lucky day')
				    ...
				    $mdDialog.show(confirm).then(function () {
				      $scope.status = 'SI';
				      $scope.activarmsMeta(ev, msMetaBk);
				    }, function () {
				      $scope.status = 'NO';
				    });
				  };
...
//	if($scope.isArray($scope.listaAnnio)){
//		var obj = $scope.listaAnnio.find(o => o.id === newValue);
...
//SELECT INI
	...
	$scope.loadListaPrtParametrosIdTipoServicio=function(){
		...
		//CARGAR DATOS DEL SIGUIENTE SELECT
//	if($scope.isArray($scope.listaPrtParametrosIdTipoServicio)){
//		var obj = $scope.listaPrtParametrosIdTipoServicio.find(o => o.id === newValue);
//		if($scope.isObject(obj)){
