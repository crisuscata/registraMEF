...
var descargarUrl = contexto+"/rs/ctrlmsPaises/descargar/";

///URLs CARGA DE ARCHIVOS
var insertDocUrl = contexto+"/rs/ctrlmsPaises/insertarchivo";
var descargarUrl = contexto+"/rs/ctrlmsPaises/descargar/";
///FIN URLs CARGA DE ARCHIVOS

var descargarvistaUrl = contexto+"/rs/ctrlmsPaises/descargarvista";
...
myapp.controller('ctrlListamsPaises', ['$mdEditDialog', '$scope', '$timeout', '$http', '$mdDialog','$location','$routeParams', '$mdPanel',  function ($mdEditDialog, $scope, $timeout, $http, $mdDialog, $location, $routeParams, $mdPanel) {
	...
	$scope.loadmsPaisess = function () {
	    //$scope.promise = $timeout(function () {
	    	...
	    	$scope.promise = $http.get(surl).then(function(res){
	    		 ...
	    		 console.log("Tiempo respuesta BD msPaises " +tiempoenBD+" Tiempo en Paginar "+tiempoenproceso);
				},
	  ...
	  $scope.getURLParametros=function(){
		  ...
		  return listamsPaisesUrl+$scope.getURLParametros();
	  }
	  
	  $scope.editmsPaises = function () {
		    $scope.nuevo = true;
		    var idpais = $routeParams.idpais;
		    if(idpais){
		    	$scope.cargarmsPaises(idpais);
		    }
		   ///CARGAR COMPLEMENTOS 
		  };
  ...
  acronimo: null,
	  ...
	  $scope.clearmsPaises = function(){
		    $scope.msPaisesModelo.idpais = null;
		    $scope.msPaisesModelo.paisNombre = null;
		    $scope.msPaisesModelo.acronimo = null;
		    
                    // ADICIONALES
	            $scope.msPaisesModelo.estadoTxt = null;

		    $scope.msPaisesModelo.editopcion = 1;
	 } 
	 
	  $scope.setMsPaisesModelo = function(msPaisesBk) {
		  $scope.msPaisesModelo.idpais = msPaisesBk.idpais;
			$scope.msPaisesModelo.paisNombre = msPaisesBk.paisNombre;
			$scope.msPaisesModelo.acronimo = msPaisesBk.acronimo;
			
                        // ADICIONALES
	                $scope.msPaisesModelo.estadoTxt = msPaisesBk.estadoTxt;
	  ...
	  $scope.editarMsPaises = function(ev, msPaisesBk) {		  
			...
			$location.url('/editar/' + $scope.msPaisesModelo.idpais);
			$scope.nuevo = false;
	  }
...
//		    				$scope.datos.push(dato); 
		    				...
		    				$mdDialog.show(
							        ...
							        .parent(angular.element(document.body))
							        .clickOutsideToClose(true)
							        .title('Guardar Países')
							        .textContent("del Países se guardó correctamente.")
							        .ariaLabel('ERROR')
							        .ok('OK')
							        .targetEvent(ev)
							    );
	   ...
	   $scope.eliminarmsPaises = function(ev,msPaisesBk){		
	        		...
	        		$http.post(eliminarmsPaisesUrl,datainsert,{headers: {'Content-Type': 'application/json'}}).then(function(res){
				        ...
				        var index = $scope.datos.findIndex(obj => obj.idpais === dato.idpais);
		...
		$scope.cargarmsPaises = function(idpais){		
			var surl = editarmsPaisesUrl+idpais;
...
///FIN ADICIONALES			 			 
	// ////////////////////////////////////////////////////////////////
		 ...
		 // Returns if a value is really a number
		    ...
		    $scope.nuevo = ($scope.isNumber($scope.msPaisesModelo.idpais) && $scope.msPaisesModelo.idpais > 0);
			$scope.edit  = true;
