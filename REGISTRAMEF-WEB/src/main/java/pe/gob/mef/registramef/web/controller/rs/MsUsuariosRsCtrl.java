...
import pe.gob.mef.registramef.web.controller.MsUsuariosData;
import pe.gob.mef.registramef.web.controller.rs.data.MsUsuariosJS;
import pe.gob.mef.registramef.web.controller.rs.data.MsUsuariosLC;
...
public class MsUsuariosRsCtrl {
	...
	public Response listarMsUsuarios(@Context HttpServletRequest req, @Context HttpServletResponse res,
		...
		if (!req.isUserInRole(Roles.ADMINISTRADOR) && !req.isUserInRole(Roles.MSUSUARIOS_CREA)
				&& !req.isUserInRole(Roles.MSUSUARIOS_VE))
			return Response.status(HttpURLConnection.HTTP_UNAUTHORIZED)
					.entity(new GenericEntity<RespuestaError>(
		...
		try {
			...
			String idSistAdmiTxt = req.getParameter("idSistAdmiTxt");
			...
			MsUsuariosFiltro msUsuariosFiltro = new MsUsuariosFiltro(idusuario, dni, username, apellidoPaterno,
					apellidoMaterno, nombres, correo, idSistAdmi, idSede, fechaModif, idSistAdmiTxt, idSedeTxt, estadoTxt, roles, iestado);
			...
			List<MsUsuariosBk> msUsuariossss = msUsuariosData.getMsUsuariosActivos(servicio,
			...
			List<MsUsuariosBk> msUsuariossssData = new ArrayList<MsUsuariosBk>();
			if (msUsuariosFiltro.isActivo()) {
				// filter
				// int contador = 0;
				for (MsUsuariosBk msUsuariosAct : msUsuariossss) {
					boolean match = true;
					Field camposdea[] = msUsuariosFiltro.getClass().getDeclaredFields();
					// if(msUsuariosAct.getIdusuario.longValue()==56L){
					...
					for (int i = 0; i < camposdea.length; i++) {
						...
						try {
							...
							if (filtroValue == null)
								...
								continue;
							Method claseMethod = msUsuariosAct.getClass().getMethod(claseGetMetod, types);
							Object claseValue = claseMethod.invoke(msUsuariosAct, new Object[0]);
							if (claseValue != null) {
					...
					if (match) {
						msUsuariossssData.add(msUsuariosAct);
					}
				}
			} else {
				msUsuariossssData = msUsuariossss;
			}
			// sort
			// System.setProperty("java.util.Arrays.useLegacyMergeSort",
			// "true");
			if (sorder != null && sorder.trim().length() > 1) {
				Collections.sort(msUsuariossssData, new Comparator<MsUsuariosBk>() {
					@SuppressWarnings({ "unchecked", "rawtypes" })
					public int compare(MsUsuariosBk msUsuarios1, MsUsuariosBk msUsuarios2) {
						...
						try {
							...
							Object value1 = method.invoke(msUsuarios1, new Object[0]);
							Object value2 = method.invoke(msUsuarios2, new Object[0]);
			...
			// rowCount
			int dataSize = msUsuariossssData.size();
			int pageSize = 100;
			try {
			...
			if (dataSize > pageSize) {
				...
				try {
					msUsuariosLC.setData(msUsuariossssData.subList(iniciodelista, iniciodelista + pageSize));
				} catch (IndexOutOfBoundsException e) {
					msUsuariosLC
							.setData(msUsuariossssData.subList(iniciodelista, iniciodelista + (dataSize % pageSize)));
				}
			} else {
				msUsuariosLC.setData(msUsuariossssData);
			}
	...
	public Response salvarMsUsuarios(@Context HttpServletRequest req, @Context HttpServletResponse res,
		...
		MsUsuariosBk msUsuariosC = new MsUsuariosBk();
		FuncionesStaticas.copyPropertiesObject(msUsuariosC, msUsuariosJS);

		if(msUsuariosJS.getNoEsEliminado()){
			msUsuariosC.setEstado(Estado.ACTIVO.getValor());
		}else{
			msUsuariosC.setEstado(Estado.ELIMINADO.getValor());
		}
		
		try {
			msUsuariosC = servicio.saveorupdateMsUsuariosBk(msUsuariosC, msUsuariosBk.getUsername(),
					msUsuariosBk.getIdusuario(), null, null, adressRemoto, false);

			// msUsuariosJS = new MsUsuariosJS();
			// FuncionesStaticas.copyPropertiesObject(msUsuariosJS,
			// msUsuariosC);
			// msUsuariosJS.setEditopcion(msUsuariosC.getmsUsuariosACL().getEditopcion());
			...
			msUsuariosData.add(servicio, msUsuariosBk.getIdusuario(), msUsuariosC);

			GenericEntity<MsUsuariosBk> registrors = new GenericEntity<MsUsuariosBk>(msUsuariosC) {
			};
	...
	public Response eliminarMsUsuarios(@Context HttpServletRequest req, @Context HttpServletResponse res,
			@HeaderParam("authorization") String authString, MsUsuariosJS msUsuariosE) {
		SpringBeanAutowiringSupport.processInjectionBasedOnCurrentContext(this);
		Principal usuario = req.getUserPrincipal();
		MsUsuariosBk msUsuariosBk = servicio.getMsUsuariosBkXUsername(usuario.getName());
		...
		String adressRemoto = getRemoteAdress(req);
		MsUsuariosBk msUsuariosC = new MsUsuariosBk();
		FuncionesStaticas.copyPropertiesObject(msUsuariosC, msUsuariosE);

		try {
			servicio.deleteMsUsuarios(msUsuariosC, msUsuariosBk.getUsername(), msUsuariosBk.getIdusuario(), null,
					adressRemoto);
			...
			msUsuariosData.refrescar(servicio, msUsuariosBk.getIdusuario());

			GenericEntity<MsUsuariosBk> registro = new GenericEntity<MsUsuariosBk>(msUsuariosC) {
			};
	...
	@POST
	@Path("/activarmsUsuarios")
	@Produces(MediaType.APPLICATION_JSON)
	public Response activarMsUsuarios(@Context HttpServletRequest req, @Context HttpServletResponse res,
			@HeaderParam("authorization") String authString, MsUsuariosJS msUsuariosE) {
		SpringBeanAutowiringSupport.processInjectionBasedOnCurrentContext(this);
		Principal usuario = req.getUserPrincipal();
		MsUsuariosBk msUsuariosBk = servicio.getMsUsuariosBkXUsername(usuario.getName());
		...
		try {
			servicio.activarMsUsuarios(msUsuariosC, msUsuariosBk.getUsername(), msUsuariosBk.getIdusuario(), null, adressRemoto);
	...
	@POST
	@Path("/eliminarListamsUsuarios")
	@Produces(MediaType.TEXT_PLAIN)
	public Response eliminarListaMsUsuarios(@Context HttpServletRequest req, @Context HttpServletResponse res,
			@HeaderParam("authorization") String authString, List<Long> msUsuariosE) {
		SpringBeanAutowiringSupport.processInjectionBasedOnCurrentContext(this);
		Principal usuario = req.getUserPrincipal();
		MsUsuariosBk msUsuariosBk = servicio.getMsUsuariosBkXUsername(usuario.getName());
		...
		if (msUsuariosE == null || msUsuariosE.isEmpty())
		...
		try {
			int tamanio = msUsuariosE.size();
			for (Long idusuario : msUsuariosE) {
				MsUsuariosBk msUsuariosC = servicio.getMsUsuariosBkXid(idusuario, msUsuariosBk.getIdusuario());
				servicio.deleteMsUsuarios(msUsuariosC, msUsuariosBk.getUsername(), msUsuariosBk.getIdusuario(), null,
						adressRemoto);
			}
			MsUsuariosData msUsuariosData = (MsUsuariosData) req.getSession().getAttribute("MsUsuariosData");
	...
	@GET
	@Path("/editarmsUsuarios/{idusuario}")
	@Produces(MediaType.APPLICATION_JSON)
	public Response editarMsUsuarios(@Context HttpServletRequest req, @Context HttpServletResponse res,
		...
		try {
			MsUsuariosBk msUsuariosE = servicio.getMsUsuariosBkXid(idusuario, msUsuariosBk.getIdusuario());

			GenericEntity<MsUsuariosBk> registro = new GenericEntity<MsUsuariosBk>(msUsuariosE) {
			};
			return Response.status(HttpURLConnection.HTTP_OK).entity(registro).build();
		} catch (Exception e) {
			// e.printStackTrace();
	...
	@Produces(MediaType.APPLICATION_JSON)
	public Response listaPrtParametrosIdparametroIdCargo(@Context HttpServletRequest req,
			@Context HttpServletResponse res, @HeaderParam("authorization") String authString) {
		SpringBeanAutowiringSupport.processInjectionBasedOnCurrentContext(this);
	...
	@GET
	@Path("/listaPrtParametrosIdparametroIdCondlabr")
	@Produces(MediaType.APPLICATION_JSON)
	public Response listaPrtParametrosIdparametroIdCondlabr(@Context HttpServletRequest req,
			@Context HttpServletResponse res, @HeaderParam("authorization") String authString) {
		SpringBeanAutowiringSupport.processInjectionBasedOnCurrentContext(this);
		...
		try {
			List<IDValorDto> datos = servicio.getPrtParametrosIdparametroIdCondlabr();
			GenericEntity<List<IDValorDto>> registrosx = new GenericEntity<List<IDValorDto>>(datos) {
			};
			return Response.status(HttpURLConnection.HTTP_OK).entity(registrosx).build();
		} catch (Exception e) {
	...
	public Response descargarFile(@Context HttpServletRequest req, @Context HttpServletResponse res,
		...
		// filtrosaplicados.append(Messages.getStringToKey("msUsuarios."+camponame)).append("=").append(filtroValue).append("
		// ");
		...
		try {
				...
				for (MsUsuariosBk msUsuariosAct : msUsuariossss) {
					...
					for (int i = 0; i < camposdea.length; i++) {
						...
						try {
							...
							if (primerregistro)
								filtrosaplicados.append(Messages.getStringToKey("msUsuarios." + camponame)).append("=")
			...
			String titulo = "USUARIOS INTERNOS" + "\nTOTAL DE REGISTROS " + dataSize;
			...
			List<String> caposvista = Arrays.asList("idusuario","dni","username","apellidoPaterno","apellidoMaterno","correo","idSistAdmiTxt","idSedeTxt","roles","fechaModif","estadoTxt");
			...
			for (String camponame : caposvista) {
				...
				cellxX.setCellValue(Messages.getStringToKey("msUsuarios." + camponame));
				tituloscontador++;
			}

			int contador = 7;
			int contadorfor = 1;
			for (MsUsuariosBk msUsuariosBkAct : msUsuariossssData) {
				...
				if (msUsuariosBkAct.getCclase() != null) {
					if (msUsuariosBkAct.getCclase().equals("cverde")) {
						cellAAX.setCellStyle(cellStyleV);
					} else if (msUsuariosBkAct.getCclase().equals("camarillo")) {
						cellAAX.setCellStyle(cellStyleA);
					} else if (msUsuariosBkAct.getCclase().equals("crojo")) {
	...
	public Response refrescarMsUsuarios(@Context HttpServletRequest req, @Context HttpServletResponse res,
		...
		try {
			MsUsuariosBk msUsuariosBkE = servicio.getMsUsuariosBkXid(idusuario, msUsuariosBk.getIdusuario());
	...
	@GET
	@Path("/listaTdSedesSedeNombreSedeSede")
	@Produces(MediaType.APPLICATION_JSON)
	public Response listaTdSedesSedeIdsedeIdsede(@Context HttpServletRequest req, @Context HttpServletResponse res,
			@HeaderParam("authorization") String authString) {
		SpringBeanAutowiringSupport.processInjectionBasedOnCurrentContext(this);
	...
	@GET
	@Path("/listadeperfiles")
	@Produces(MediaType.APPLICATION_JSON)
	public Response listaPerfiles(@Context HttpServletRequest req, @Context HttpServletResponse res,
			@HeaderParam("authorization") String authString) {
		SpringBeanAutowiringSupport.processInjectionBasedOnCurrentContext(this);
		...
		try {
			List<String> perfiles = Roles.listaPerfiles;
			GenericEntity<List<String>> registrosx = new GenericEntity<List<String>>(perfiles) {
			};
			return Response.status(200).entity(registrosx).build();
		} catch (Exception e) {
			String mensaje = e.getMessage();
	...
	public Response ubigeoXDefecto(@Context HttpServletRequest req, @Context HttpServletResponse res,
		...
		try {
			...
			UbigeoXDefectoJS ubigeoXDefectoJS = new UbigeoXDefectoJS(xDefectoCoddpto, xDefectoCodprov, xDefectoCoddist);
