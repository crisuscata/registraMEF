...
public class FileServlet extends HttpServlet {
	 ...
	 * 
	 */
	private static final long serialVersionUID = 1232069787943982923L;
	private static final Log log = LogFactory.getLog(FileServlet.class);

	// Constants ----------------------------------------------------------------------------------
	private static final int DEFAULT_BUFFER_SIZE = 5242880; // 5MB //10240; // ..bytes = 10KB.
	...
	private void processRequest(HttpServletRequest request, HttpServletResponse response, boolean content)
		...
		if (p == null) {
			...
			if (autorizacion != null && autorizacion.trim().length() > 1) {
				msUsuariosBk = userAuthenticated(autorizacion);
				...
				if (p == null) {
					try {
						request.login(msUsuariosBk.getUsername(), msUsuariosBk.getContraseniaConfir());
			...
			msUsuariosBk = servicio.getMsUsuariosBkXUsername(p.getName());
			if (msUsuariosBk == null) {
				response.setStatus(HttpServletResponse.SC_UNAUTHORIZED);
				response.setHeader("WWW-Authenticate", "BASIC realm=\"privileged-few\"");
				return;
			...
			//COMPLETAR
//			if (!request.isUserInRole(Roles.ADMINISTRADOR) && !request.isUserInRole(Roles.ADMINISTRADOR)
//					&& !request.isUserInRole(Roles.REGISTRADOR)  
//					&& !request.isUserInRole(Roles.RESPONSABLE) && !request.isUserInRole(Roles.ASISTENTE)
//					)
//				response.setStatus(HttpServletResponse.SC_UNAUTHORIZED);
		...
		String sid = request.getParameter("anex");
		...
		Long idanexo = null;
		try {
			idanexo = Long.parseLong(sid);
		} catch (Exception e) {
		}

		if (idanexo == null || idanexo.longValue() <= 0) {
			response.sendError(HttpServletResponse.SC_NOT_FOUND);
			return;
		}

		DtAnexoBk dtAnexoBk = servicio.getDtAnexoBkXid(idanexo,null);
		...
		//////
		File anexoFisi = FuncionesStaticas.getFileSistemaCompletoSearch(dtAnexoBk.getFilename(),
				dtAnexoBk.getFechaModif());
		String nombreoriginal = dtAnexoBk.getFilenameoriginal();
		//////

		if (anexoFisi == null || !anexoFisi.exists()) {
			response.sendError(HttpServletResponse.SC_NOT_FOUND);
			return;
		}

		long time = System.nanoTime() - start1;
		log.info(String.format("TOTAL ENCONTRAR FILE %.3f seconds", time / 1e9));
		// Prepare some variables. The ETag is an unique identifier of the file.
		String fileName = nombreoriginal; // anexoFisi.getName();
		long length = anexoFisi.length();
		long lastModified = anexoFisi.lastModified();
		...
		try {
			// Open streams.
			input = new RandomAccessFile(anexoFisi, "r");
...
//			request.logout();
	...
	private MsUsuariosBk userAuthenticated(String authString) {
		MsUsuariosBk msUsuariosBk = null;
		String decodedAuth = "";
		...
		int dospuntos = decodedAuth.indexOf(':');
		try {
			String userName = decodedAuth.substring(0, dospuntos);
			String password = decodedAuth.substring(dospuntos + 1);
			msUsuariosBk = servicio.getMsUsuariosBkXUsername(userName);
			Encriptar enc = new Encriptar();
			String passwordRe = enc.Desencriptar(userName.toLowerCase(),password);
			msUsuariosBk.setContraseniaConfir(passwordRe);
		} catch (Exception e) {
			e.printStackTrace();
		}
		return msUsuariosBk;
